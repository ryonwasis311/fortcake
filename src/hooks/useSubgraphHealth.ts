...
interface NftMarketSubgraphHealthResponse {
...
const useSubgraphHealth = () => {
    ...
    useEffect(() => {
        const getSubgraphHealth = async () => {
            try {
                const { indexingStatusForCurrentVersion } = await request<NftMarketSubgraphHealthResponse>(
                    GRAPH_HEALTH,
                    gql`
                ...
                const chainHeadBlock = indexingStatusForCurrentVersion.chains[0].chainHeadBlock.number
                const latestBlock = indexingStatusForCurrentVersion.chains[0].latestBlock.number
