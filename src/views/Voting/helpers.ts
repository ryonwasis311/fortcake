...
export const calculateVoteResults = (votes: Vote[]): { [key: string]: Vote[] } => {
    return votes.reduce((accum, vote) => {
        ...
        return {
            ...accum,
            [choiceText]: accum[choiceText as keyof typeof accum] ? [...accum[choiceText as keyof typeof accum], vote] : [vote],
        }
    }, {})
}
...
export const getTotalFromVotes = (votes: Vote[]) => {
    return votes.reduce((accum, vote) => {
        let power = parseFloat(vote.metadata?.votingPower as string)
