...
import { Auction, AuctionStatus, Bidder, ConnectedBidder } from '../../../../config/constants/types'
...
const AuctionDetails: React.FC<AuctionDetailsProps> = ({ auction, connectedBidder, refreshBidders }) => {
    ...
    const getBidSection = () => {
        ...
        if (whitelistedAndReadyToBid) {
            const { amount, position } = connectedBidder.bidderData as Bidder
            return (
                <>
                    <Tag outline variant="success" startIcon={<CheckmarkCircleIcon />} >
                        {t('Connected as %projectName%', { projectName: connectedBidder.bidderData?.tokenName ? connectedBidder.bidderData?.tokenName : "" })}
                    </Tag>
