...
const PlaceBidModal: React.FC<PlaceBidModalProps> = ({
    ...
    const { amount, position } = bidderData as Bidder
    const isFirstBid = amount.isZero()
    const isInvalidFirstBid = isFirstBid && !isMoreThanInitialBidAmount

    useEffect(() => {
        setIsMoreThanInitialBidAmount(parseFloat(bid) >= (initialBidAmount ? initialBidAmount : 0))
    ...
    useEffect(() => {
        if (userNotEnoughCake) {
            ...
            setErrorText(t('First bid must be %initialBidAmount% CAKE or more.', { initialBidAmount: (initialBidAmount ? initialBidAmount : 0) }))
    ...
    const { isApproving, isApproved, isConfirmed, isConfirming, handleApprove, handleConfirm } =
        useApproveConfirmTransaction({
            ...
            onApprove: () => {
                return callWithGasPrice(cakeContract, 'approve', [farmAuctionContract.address, ethers.constants.MaxUint256], null)
            },
            onApproveSuccess: async ({ receipt }) => {
                toastSuccess(
                    t('Contract approved - you can now place your bid!'),
                    <ToastDescriptionWithTx txHash={receipt.transactionHash} children={undefined} />,
                )
            },
            onConfirm: () => {
                const bidAmount = new BigNumber(bid).times(DEFAULT_TOKEN_DECIMAL).toString()
                return callWithGasPrice(farmAuctionContract, 'bid', [bidAmount], null)
            },
            onSuccess: async ({ receipt }) => {
                refreshBidders()
                onDismiss && onDismiss()
                toastSuccess(t('Bid placed!'), <ToastDescriptionWithTx txHash={receipt.transactionHash} children={undefined} />)
            },
        })
    ...
    return (
        <StyledModal title={t('Place a Bid')} onDismiss={onDismiss} headerBackground={theme?.colors.gradients.cardHeader}>
            ...
            <InnerContent>
                ...
                {isFirstBid && (
                    <Text pb="8px" small>
                        {t('First bid must be %initialBidAmount% CAKE or more.', { initialBidAmount: (initialBidAmount ? initialBidAmount : 0) })}
                    </Text>
                )}
                <BalanceInput
