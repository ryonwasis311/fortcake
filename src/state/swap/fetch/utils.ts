...
export const getIdsByTimeWindow = ({
    ...
    // return times(idsCount, (value) => `${pairAddress}-${pairLastIdAsNumber - value * (timeWindowGapMapping ? timeWindowGapMapping[timeWindow] : 0)}`)
    return times(idsCount, (value) => {
        const timeWindowGap = timeWindowGapMapping?.[timeWindow] ?? 0;
        return `${pairAddress}-${pairLastIdAsNumber - value * timeWindowGap}`;
    });
