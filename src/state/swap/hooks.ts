...
export function useDerivedSwapInfo(): {
    ...
    if (balanceIn && amountIn && balanceIn.lessThan(amountIn)) {
        inputError = t('Insufficient %symbol% balance', { symbol: amountIn.currency.symbol as string })
    }
...
export const useFetchPairPrices = ({
        ...
        const updatePairId = () => {
            try {
                ...
                if (pairAddress !== pairId) {
                    setPairId(pairAddress as string)
                }
            } catch (error) {
                setPairId("")
            }
    ...
    // let pairPrices: PairPricesNormalized  = hasNoDirectData && hasNoDerivedData ? [] : undefined;
    let pairPrices: PairPricesNormalized = [];
